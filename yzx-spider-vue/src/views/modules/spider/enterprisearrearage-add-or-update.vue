<template>
  <el-dialog
    ...
    <el-form-item label="欠费类型" prop="costType">
      <el-input v-model="dataForm.costType" placeholder="欠费类型"></el-input>
    </el-form-item>
    <el-form-item label="年月份" prop="yearMonth">
      <!-- <el-input v-model="dataForm.yearMonth" placeholder="年月份"></el-input> -->
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          costType: [
            { required: true, message: '欠费类型不能为空', trigger: 'blur' }
          ],
          yearMonth: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/spider/enterprisearrearage/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterpriseId = data.enterpriseArrearage.enterpriseId
                this.dataForm.incubatorEnterpriseId = data.enterpriseArrearage.incubatorEnterpriseId
                this.dataForm.incubatorName = data.enterpriseArrearage.incubatorName
                this.dataForm.enterpriseName = data.enterpriseArrearage.enterpriseName
                this.dataForm.costType = data.enterpriseArrearage.costType
                this.dataForm.yearMonth = data.enterpriseArrearage.yearMonth
                this.dataForm.costLimitType = data.enterpriseArrearage.costLimitType
                this.dataForm.quantity = data.enterpriseArrearage.quantity
                this.dataForm.remark = data.enterpriseArrearage.remark
                this.dataForm.createtime = data.enterpriseArrearage.createtime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/spider/enterprisearrearage/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
