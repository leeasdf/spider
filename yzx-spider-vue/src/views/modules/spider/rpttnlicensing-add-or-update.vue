    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/spider/rpttnlicensing/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterpriseId = data.rptTnLicensing.enterpriseId
                this.dataForm.entName = data.rptTnLicensing.entName
                this.dataForm.licenseNo = data.rptTnLicensing.licenseNo
                this.dataForm.licenseName = data.rptTnLicensing.licenseName
                this.dataForm.validateFrom = data.rptTnLicensing.validateFrom
                this.dataForm.validateTo = data.rptTnLicensing.validateTo
                this.dataForm.authority = data.rptTnLicensing.authority
                this.dataForm.content = data.rptTnLicensing.content
                this.dataForm.creator = data.rptTnLicensing.creator
                this.dataForm.createTime = data.rptTnLicensing.createTime
                this.dataForm.updater = data.rptTnLicensing.updater
                this.dataForm.updateTime = data.rptTnLicensing.updateTime
                this.dataForm.dataState = data.rptTnLicensing.dataState
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/spider/rpttnlicensing/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
