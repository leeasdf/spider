  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('spider:xylgdsjcitytaxowing:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('spider:xylgdsjcitytaxowing:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      <el-table-column
        ...
        <template slot-scope="props">
          <el-form inline label-width="200px" class="yzx-table-expand">
            <el-form-item label="法人（业主）姓名"><span>{{props.row.legalPersonName}}</span></el-form-item>
            <el-form-item label="法人（业主）身份证号"><span>{{props.row.legalPersonCertNo}}</span></el-form-item>
            <el-form-item label="经营地址"><span>{{props.row.businessAddress}}</span></el-form-item>
            <el-form-item label="创建时间"><span>{{props.row.createTime}}</span></el-form-item>
            <el-form-item label="房产税-欠税余额"><span>{{props.row.propertyTaxOwing}}</span></el-form-item>
            <el-form-item label="房产税-其中本年新欠"><span>{{props.row.propertyTaxOwingNew}}</span></el-form-item>
            <el-form-item label="城市维护建设税-欠税余额"><span>{{props.row.cityBuildTax}}</span></el-form-item>
            ...
            <el-form-item label="契税-其中本年新欠"><span>{{props.row.contractTax}}</span></el-form-item>
            ...
            <el-form-item label="数据更新时间戳"><span>{{props.row.dataUpdateTime}}</span></el-form-item>
      ...
      <!-- <el-table-column
        prop="propertyTaxOwing"
        header-align="center"
        align="center"
        label="房产税-欠税余额">
      </el-table-column>
      <el-table-column
        prop="propertyTaxOwingNew"
        header-align="center"
        align="center"
        label="房产税-其中本年新欠">
      </el-table-column>
...
<script>
  import AddOrUpdate from './xylgdsjcitytaxowing-add-or-update'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/spider/xylgdsjcitytaxowing/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/spider/xylgdsjcitytaxowing/delete'),
            method: 'post',
