    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/spider/rpttnreportwebsite/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterpriseId = data.rptTnReportWebsite.enterpriseId
                this.dataForm.entName = data.rptTnReportWebsite.entName
                this.dataForm.reportId = data.rptTnReportWebsite.reportId
                this.dataForm.name = data.rptTnReportWebsite.name
                this.dataForm.type = data.rptTnReportWebsite.type
                this.dataForm.url = data.rptTnReportWebsite.url
                this.dataForm.creator = data.rptTnReportWebsite.creator
                this.dataForm.createTime = data.rptTnReportWebsite.createTime
                this.dataForm.updater = data.rptTnReportWebsite.updater
                this.dataForm.updateTime = data.rptTnReportWebsite.updateTime
                this.dataForm.dataState = data.rptTnReportWebsite.dataState
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/spider/rpttnreportwebsite/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
