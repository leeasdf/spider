    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/spider/rpttnillegal/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterpriseId = data.rptTnIllegal.enterpriseId
                this.dataForm.entName = data.rptTnIllegal.entName
                this.dataForm.date = data.rptTnIllegal.date
                this.dataForm.reason = data.rptTnIllegal.reason
                this.dataForm.agency = data.rptTnIllegal.agency
                this.dataForm.creator = data.rptTnIllegal.creator
                this.dataForm.createTime = data.rptTnIllegal.createTime
                this.dataForm.updater = data.rptTnIllegal.updater
                this.dataForm.updateTime = data.rptTnIllegal.updateTime
                this.dataForm.dataState = data.rptTnIllegal.dataState
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/spider/rpttnillegal/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
