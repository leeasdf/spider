    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.zjid) {
            this.$http({
              url: this.$http.adornUrl(`/spider/cssfgwaicinfofinance/info/${this.dataForm.zjid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.isinvalid = data.csSfgwAicInfofinance.isinvalid
                this.dataForm.lastdate = data.csSfgwAicInfofinance.lastdate
                this.dataForm.etpsId = data.csSfgwAicInfofinance.etpsId
                this.dataForm.name = data.csSfgwAicInfofinance.name
                this.dataForm.certype = data.csSfgwAicInfofinance.certype
                this.dataForm.cerno = data.csSfgwAicInfofinance.cerno
                this.dataForm.fixedtel = data.csSfgwAicInfofinance.fixedtel
                this.dataForm.mobilephone = data.csSfgwAicInfofinance.mobilephone
                this.dataForm.email = data.csSfgwAicInfofinance.email
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/spider/cssfgwaicinfofinance/${!this.dataForm.zjid ? 'save' : 'update'}`),
              method: 'post',
