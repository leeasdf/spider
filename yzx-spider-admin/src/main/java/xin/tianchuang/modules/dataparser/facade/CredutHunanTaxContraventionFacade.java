 ...
 */
@Component
public class CredutHunanTaxContraventionFacade {

    private static final Logger LOGGER= LoggerFactory.getLogger(CredutHunanTaxContraventionFacade.class);
    @Autowired
    private SpiderRawDataService spiderRawDataService;

    @Autowired
    private CredithunanTaxContraventionService service;
    ...
    public void doBussiness(String enterpriseName){
        ...
        try {
            for (SpiderRawDataEntity entity : entityList) {
                CredithunanTaxContraventionEntity taxa=parser.parser(entity.getContent());
    ...
    public void doBussiness(SpiderRawDataEntity dataEntity) throws Exception{
        ...
        Parser<CredithunanTaxContraventionEntity> parser= ParserFactory.getInstance(SpiderTypeEnum.MAJOR_TAX_OFFENCES);
        try {
            CredithunanTaxContraventionEntity entity=parser.parser(dataEntity.getContent());
